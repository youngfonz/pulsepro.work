              ...
              {overdueGroups.map((group) => (
                <div key={group.id || '__standalone__'}>
                  <Link
                    ...
                    className="flex items-center justify-between gap-2 px-3 sm:px-5 pt-3 pb-1 hover:bg-muted/30 transition-colors"
                  >
                  ...
                  {group.tasks.map((task) => (
                    <Link
                      key={task.id}
                      href={`/tasks/${task.id}`}
                      className="block px-3 sm:px-5 py-2.5 hover:bg-muted/50 transition-colors"
                    >
            ...
            {tasksDueToday.length === 0 && projectsDueThisWeek.length === 0 ? (
              ...
              <div>
                {tasksDueToday.length > 0 && (
                  <div>
                    <p className="px-3 sm:px-5 pt-2 pb-1 text-xs font-medium uppercase tracking-wider text-muted-foreground">Due Today</p>
                    <div className="divide-y divide-border">
                      {tasksDueToday.map((task) => (
                        <Link
                          key={task.id}
                          href={`/tasks/${task.id}`}
                          className="flex items-center gap-3 px-3 sm:px-5 py-2.5 hover:bg-muted transition-colors"
                        >
                ...
                {projectsDueThisWeek.length > 0 && (
                  <div className={tasksDueToday.length > 0 ? 'border-t border-border' : ''}>
                    <p className="px-3 sm:px-5 pt-3 pb-1 text-xs font-medium uppercase tracking-wider text-muted-foreground">Projects This Week</p>
                    <div className="divide-y divide-border">
            ...
            <div className="divide-y divide-border">
              {projectHealth.slice(0, 6).map((project) => (
                <Link
                  ...
                  href={project.href}
                  className="flex items-center gap-3 px-3 sm:px-5 py-3 hover:bg-muted/50 transition-colors"
                >
                  <span
        ...
        <Card className="h-full">
          ...
          <CardContent className="p-0">
            {recentlyViewed.length === 0 ? (
              <Link
                href="/projects/new"
                className="block px-3 sm:px-5 py-8 text-center text-muted-foreground hover:bg-muted transition-colors"
              >
              ...
              <div className="divide-y divide-border">
                {recentlyViewed.map((item) => (
                  <Link
                    ...
                    href={item.href}
                    className="flex items-center gap-3 px-3 sm:px-5 py-3 hover:bg-muted/50 transition-colors active:bg-muted"
                  >
